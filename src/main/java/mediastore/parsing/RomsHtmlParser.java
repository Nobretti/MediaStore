...
public class RomsHtmlParser {
	...
	final static String DB_NAME = "MediaStore";

	public static void main(String[] args) {

		File htmlFiles = new File("src/main/resources/wikidata/");

		int indexFile = 2;
		
		for(File htmlFile: htmlFiles.listFiles()) {

			String filePureName = htmlFile.getName().replaceFirst("[.][^.]+$", "");
			String newFileName = indexFile++ + "-" + filePureName;
			
			try (FileOutputStream fos = new FileOutputStream("src/docker/mysql-dump/" + newFileName +".sql")){

				fos.write(("use " + DB_NAME +";\n").getBytes());
